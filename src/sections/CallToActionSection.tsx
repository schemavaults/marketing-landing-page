...
export function CTASection({ Link }: CTASectionProps): ReactElement {
  ...
  const heading: string = privateBeta
    ? "Ready to simplify how you work with data?"
    : "Start shipping with type-safe data today.";
        ...
        <div className="max-w-4xl mx-auto text-center space-y-8">
          <div className="space-y-4">
            ...
            <p className="mx-auto max-w-[600px] text-muted-foreground text-lg">
              {privateBeta ? (
                <>
                  Start building with <Wordmark /> as soon as it&apos;s
                  available to the public. Drop your email below and
                  we&apos;ll let you know the moment early access opens.
                </>
              ) : (
                <>
                  Create a free <Wordmark /> account in under a minute, or
                  join the mailing list to follow what we&apos;re building.
                </>
              )}
            </p>
          </div>
          ...
          <p className="text-sm text-muted-foreground">
            Free tier forever • No credit card required • Cancel anytime
          </p>
        </div>
      </div>
    </section>
